...
import { resetAndImportSeedData, resetAndImportSeedDataFromJson } from './lib/seed-data-import';
...
export default function Home() {
  ...
  const handleJsonImport = () => {
    ...
    if (!jsonImportText.trim()) {
      setJsonImportError('JSONを入力してください');
      return;
    }

    if (confirm('JSONからデータをインポートしますか？既存のデータはすべて削除されます。')) {
      try {
        resetAndImportSeedDataFromJson(jsonImportText);
        setJsonImportText('');
        setShowJsonImport(false);
        // ページを強制的にリロード（キャッシュをクリア）
        window.location.reload();
      } catch (error) {
        setJsonImportError(error instanceof Error ? error.message : 'インポートに失敗しました');
      }
    }
  };
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8">
              ...
              <div className="space-y-6">
                ...
                <div className="p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700">
                  <h2 className="text-xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">JSONからインポート</h2>
                  <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-4">
                    コピーしたJSONデータを貼り付けてインポートできます。既存のデータはすべて削除されます。
                  </p>
                  
                  {!showJsonImport ? (
                    <button
                      onClick={() => setShowJsonImport(true)}
                      className="w-full px-4 py-3 bg-green-600 text-white rounded hover:bg-green-700 transition-colors text-left flex items-center gap-2"
                    >
                    ...
                    <div className="space-y-4">
                      <div>
                        <textarea
                          ...
                          className="w-full h-64 p-3 border border-zinc-300 dark:border-zinc-600 rounded-md font-mono text-sm bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
                        />
                        {jsonImportError && (
                          <div className="mt-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
                            <p className="text-sm text-red-600 dark:text-red-400">{jsonImportError}</p>
                          </div>
                        )}
                      </div>
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-zinc-300 dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 rounded hover:bg-zinc-400 dark:hover:bg-zinc-600 transition-colors"
                        >
                          キャンセル
                        </button>
                      </div>
                ...
                <div className="p-6 bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700">
                  <h2 className="text-xl font-bold mb-4 text-zinc-900 dark:text-zinc-100">その他の操作</h2>
